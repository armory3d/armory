	 	...
	 	for (i in 0...lights.length) {
			...
			kha.compute.Compute.setInt(voxel_cb5, iron.RenderPath.clipmapLevel);
	 		...
	 		if (l.data.raw.type == "sun") {
	 			...
	 			kha.compute.Compute.setSampledTexture(voxel_tb5, rts.get("shadowMapAtlasTranparent").image);
	 			#else
	 			...
	 			kha.compute.Compute.setSampledTexture(voxel_tc5, rts.get("shadowMapAtlasTranparent").image);
	 			#else
				...
				kha.compute.Compute.setSampledTexture(voxel_td5, rts.get("shadowMapAtlasTranparent").image);
				#else
