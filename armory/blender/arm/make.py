...
def compile(assets_only=False):
    ...
    if state.target == "custom":
        ...
        os.makedirs(arm.utils.build_dir() + '/n64/build', exist_ok=True)
        os.makedirs(arm.utils.build_dir() + '/n64/src', exist_ok=True)

        # Get the SDK path to reference libdragon and tiny3d from Armory's lib directory
        sdk_path = arm.utils.get_sdk_path()
        libdragon_path = os.path.join(sdk_path, 'lib', 'libdragon').replace('\\', '/')
        tiny3d_path = os.path.join(sdk_path, 'lib', 'tiny3d').replace('\\', '/')

        make_file_content = f'''# N64 Makefile - Generated by Armory
BUILD_DIR=build
...
GAME_NAME := {arm.utils.safestr(wrd.arm_project_name)}
...
$(BUILD_DIR)/$(GAME_NAME).dfs: $(assets_conv)
$(BUILD_DIR)/$(GAME_NAME).elf: $(src:%.c=$(BUILD_DIR)/%.o)

$(GAME_NAME).z64: N64_ROM_TITLE=$(GAME_NAME)
$(GAME_NAME).z64: $(BUILD_DIR)/$(GAME_NAME).dfs
