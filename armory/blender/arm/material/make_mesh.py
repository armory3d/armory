...
def make_forward_base(con_mesh, parse_opacity=False, transluc_pass=False):
    ...
    if '_VoxelGI' in wrd.world_defs:
        ...
        frag.write('    indirect += traceSpecular(wposition, n, voxels, voxelsSDF, vVec, roughness * roughness, clipmaps, gl_FragCoord.xy, velocity).rgb * f0 * voxelgiRefl;')
        frag.write('}')
    ...
    if '_VoxelRefract' in wrd.world_defs and parse_opacity:
        ...
        frag.write('    vec3 refraction = traceRefraction(wposition, n, voxels, voxelsSDF, vVec, ior, roughness * roughness, clipmaps, gl_FragCoord.xy, velocity, opacity).rgb * voxelgiRefr;')
