...
void main() {
	...
	if (weights.x > 0.0) {
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.x, 0, 0), uint(light.r * weights.x * 1024));
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.x, 0, voxelgiResolution.x), uint(light.g * weights.x * 1024));
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.x, 0, voxelgiResolution.x * 2), uint(light.b * weights.x * 1024));
	}
	if (weights.y > 0.0) {
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.y, 0, 0), uint(light.r * weights.y * 1024));
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.y, 0, voxelgiResolution.x), uint(light.g * weights.y * 1024));
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.y, 0, voxelgiResolution.x * 2), uint(light.b * weights.y * 1024));
	}
	if (weights.z > 0.0) {
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.z, 0, 0), uint(light.r * weights.z * 1024));
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.z, 0, voxelgiResolution.x), uint(light.g * weights.z * 1024));
		imageAtomicMax(voxelsLight, ivec3(dst) + ivec3(face_indices.z, 0, voxelgiResolution.x * 2), uint(light.b * weights.z * 1024));
	}
}
