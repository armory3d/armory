...
vec3 calculateInvisibilityLight(vec2 uv, vec3 pos, vec3 normal, float depth) {
...
void main() {
	...
	for (int i = 0; i < ssgiSamples; i++) {
		...
		vec2 sampleUV = clamp(
			texCoord + offset * (BayerMatrix8[int(gl_FragCoord.x + velocity.x) % 8][int(gl_FragCoord.y + velocity.y) % 8] - 0.5) / screenSize,
		...
		float depthDiff = abs(pos.z - samplePos.z);
		//if (depthDiff > MAX_DEPTH_DIFFERENCE) continue;

		vec3 sampleColor = calculateInvisibilityLight(sampleUV, samplePos, sampleNormal, sampleDepth);

		float dist = length(pos - samplePos);
		float weight = 1.0 / (1.0 + dist);// * max(dot(sampleNormal, n), 0.0);
