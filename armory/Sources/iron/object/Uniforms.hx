			...
			for (j in 0...tus.length) { // Set texture
				if (samplerID == tus[j].name) {
					...
					if (isImage) {
						...
						{
							g.setTexture3DParameters(context.textureUnits[j], TextureAddressing.Clamp, TextureAddressing.Clamp, TextureAddressing.Clamp, TextureFilter.PointFilter, TextureFilter.PointFilter, MipMapFilter.PointMipFilter);
						}
						else if (rt.raw.name.startsWith("voxels"))
						{
							g.setTexture3DParameters(context.textureUnits[j], TextureAddressing.Clamp, TextureAddressing.Clamp, TextureAddressing.Clamp, LinearFilter.PointFilter, TextureFilter.LinearFilter, MipMapFilter.LinearMipFilter);
						}
						else
						{
							g.setTextureParameters(context.textureUnits[j], TextureAddressing.Clamp, TextureAddressing.Clamp, TextureFilter.LinearFilter, TextureFilter.LinearFilter, MipMapFilter.LinearMipFilter);
