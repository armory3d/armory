...
def make_forward_base(con_mesh, parse_opacity=False, transluc_pass=False):
    ...
    if '_VoxelGI' in wrd.world_defs or '_VoxelAOvar' in wrd.world_defs:
        ...
        vert.write('clipmapLevel = vec3(max(log2(dist / voxelgiResolution.x * 2.0), 0));')
        vert.write('float voxelSize = pow(2.0, int(clipmapLevel)) * 0.5;')
    ...
    if '_VoxelAOvar' in wrd.world_defs:
        if '_VoxelTemporal' in wrd.world_defs:
            ...
            frag.write('indirect *= ((1.0 - traceAO(voxpos, n, voxels, clipmapLevel.x)) * voxelBlend + (1.0 - traceAO(voxpos, n, voxelsLast, clipmapLevel.x)) * (1.0 - voxelBlend));')
        else:
            frag.write('indirect *= 1.0 - traceAO(voxpos, n, voxels, clipmapLevel.x);')

    if '_VoxelGI' in wrd.world_defs:
        if '_VoxelTemporal' in wrd.world_defs:
            frag.add_uniform('float voxelBlend', '_voxelBlend')
            frag.write('indirect += (traceDiffuse(voxpos, n, voxels, roughness, clipmapLevel.x).rgb * voxelBlend + traceDiffuse(voxpos, n, voxelsLast, roughness, clipmapLevel.x).rgb * (1.0 - voxelBlend)) * voxelgiDiff * albedo;')
        else:
            frag.write('indirect += traceDiffuse(voxpos, n, voxels, roughness, clipmapLevel.x).rgb * voxelgiDiff * albedo;')
        ...
        if '_VoxelTemporal' in wrd.world_defs:
            ...
            frag.write('indirect += traceSpecular(voxpos, n, voxels, -eyeDir, roughness, clipmapLevel.x).rgb * voxelgiRefl * specular;')
        else:
            frag.write('indirect += traceSpecular(voxpos, n, voxels, -eyeDir, roughness, clipmapLevel.x).rgb * voxelgiRefl * specular;')
    ...
    if '_VoxelRefract' in wrd.world_defs and parse_opacity:
        if '_VoxelTemporal' in wrd.world_defs:
            ...
            frag.write('vec3 refraction = traceRefraction(voxpos, n, voxels, eyeDir, ior, roughness, clipmapLevel.x) * voxelgiRefr;')
        else:
            frag.write('vec3 refraction = traceRefraction(voxpos, n, voxels, eyeDir, ior, roughness, clipmapLevel.x) * voxelgiRefr;')
