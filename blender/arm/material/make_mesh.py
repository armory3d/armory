...
def make_forward_base(con_mesh, parse_opacity=False, transluc_pass=False):
    ...
    if '_VoxelGI' in wrd.world_defs or '_VoxelAOvar' in wrd.world_defs:
        ...
        vert.write('clipmapLevel = vec3(max(log2(dist / voxelgiHalfExtents.x), 0));')
        vert.write('float voxelSize = pow(2.0, clipmapLevel.x) * 2.0 / voxelgiResolution.x;')
        vert.write('int clipmapLevelSize = int(pow(2.0, clipmapLevel.x) * voxelgiHalfExtents.x);')
    ...
    if '_SinglePoint' in wrd.world_defs:
        ...
        if '_VoxelShadow' in wrd.world_defs and ('_VoxelAOvar' in wrd.world_defs or '_VoxelGI' in wrd.world_defs):
            frag.write('  , voxels, voxpos, int(clipmapLevel.x)')
