...
def make_forward(con_mesh, rpasses):
    ...
    if '_SSRefraction' in wrd.world_defs or '_VoxelGIRefract' in wrd.world_defs:
        if parse_opacity:
            frag.write('fragColor[GBUF_IDX_REFRACTION] = vec4(rior, opacity, 0.0, 0.0);')
        else:
            frag.write('fragColor[GBUF_IDX_REFRACTION] = vec4(1.0, 1.0, 0.0, 0.0);')
...
def make_forward_base(con_mesh, parse_opacity=False, transluc_pass=False):
    ...
    if '_VoxelAOvar' in wrd.world_defs or '_VoxelGI' in wrd.world_defs:
        ...
        if '_VoxelGITemporal' in wrd.world_defs:
            ...
            frag.write('indirect *= 1.0 - (traceAO(voxpos, n, voxels) * voxelBlend + traceAO(voxpos, n, voxelsLast) * (1.0 - voxelBlend));')
        else:
