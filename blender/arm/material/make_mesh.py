...
def make_forward_base(con_mesh, parse_opacity=False, transluc_pass=False):
    ...
    if '_VoxelGI' in wrd.world_defs or '_VoxelAOvar' in wrd.world_defs:
        ...
        vert.add_out('float clipmapLevel')
        vert.write('vec3 P = vec3(W * spos);')
        ...
        vert.write('vec3 eyeSnap = floor((viewerPos + eyeLook * voxelSize / 2.0 * voxelgiHalfExtents.x) / voxelSize) * voxelSize;')
        ...
        if rpdat.arm_rp_displacement == 'Tessellation':
            ...
            make_tess.interpolate(tese, 'clipmapLevel', 1, declare_out=True)
    ...
    if '_VoxelGI' in wrd.world_defs:
        if '_VoxelTemporal' in wrd.world_defs:
            frag.add_uniform('float voxelBlend', '_voxelBlend')
            frag.write('indirect += (traceDiffuse(voxpos, n, voxels, roughness, clipmapLevel).rgb * voxelBlend + traceDiffuse(voxpos, n, voxelsLast, roughness, clipmapLevel).rgb * (1.0 - voxelBlend)) * voxelgiDiff * albedo;')
        else:
            frag.write('indirect += traceDiffuse(voxpos, n, voxels, roughness, clipmapLevel).rgb * voxelgiDiff * albedo;')
