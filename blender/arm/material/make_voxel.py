...
def make_gi(context_id):
    ...
    vert.write('vec3 eyeSnap = floor((viewerPos + normalize(eyeLook) * clipmapLevelSize) / voxelSize) * voxelSize;')
    ...
    geom.write('    clipmapOffset = (voxelSize[i] - pow(2.0, clipmapLevelGeom[i])) / (voxelgiResolution.x * pow(2.0, clipmapLevelGeom[i]));')
...
def make_ao(context_id):
    ...
    if arm.utils.get_gapi() == 'direct3d11':
        ...
        geom.write('    stage_output.clipmapOffset = (stage_input[i].voxelSize - pow(2.0, stage_input[i].clipmapLevel)) / (voxelgiResolution * pow(2.0, stage_input[i].clipmapLevel));')
        geom.write('    if (p.z > p.x && p.z > p.y) {')
