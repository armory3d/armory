...
def make_gi(context_id):
    ...
    geom.write('    voxposition = voxpositionGeom[i] + voxelSize[i] - 2.0 / voxelgiResolution.x;')
...
def make_ao(context_id):
    ...
    if arm.utils.get_gapi() == 'direct3d11':
        ...
        geom.write('    stage_output.wpos = stage_input[i].svpos.xyz + stage_input[i].voxelSize - 2.0 / voxelgiResolution.x;')
        ...
        frag.write('  voxels[(voxposition * 0.5 + 0.5) * voxelgiResolution * stage_input.wpos] = 1.0;')
        frag.write('')
        frag.write('}')
    else:
        geom.ins = vert.outs
