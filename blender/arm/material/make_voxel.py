...
def make_ao(context_id):
    ...
    if arm.utils.get_gapi() == 'direct3d11':
        ...
        vert.write('    stage_output.svpos.xyz = (mul(float4(stage_input.pos.xyz, 1.0), W).xyz - clipmap_center) / voxelSize * 1.0 / int3(' + voxRes + ', ' + voxRes + ', ' + voxResZ + ');')
        ...
        frag.write('int3 uvw = int3(stage_input.wpos * 0.5 + 0.5);')
        frag.write('uvw.y += clipmapLevel;')
        frag.write('uvw *= int3(' + voxRes + ', ' + voxRes + ', ' + voxResZ + ');')
        frag.write('  voxels[uvw] = 1.0;')
        frag.write('')
        frag.write('}')
    else:
        geom.ins = vert.outs
        ...
        vert.write('float voxelSize = voxelgiVoxelSize * pow(2.0, clipmapLevel);')
        vert.write('vec3 clipmap_center = floor(eye / (voxelSize * 2.0)) * voxelSize * 2.0;')
