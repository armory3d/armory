...
void main() {
    ...
    if (d == 1.0 || d == 0.0 || ior == 1.0 || opac == 1.0) {
    ...
    float intensity = pow(refractivity, ss_refractionFalloffExp) * screenEdgeFactor * clamp(length(viewPos - hitCoord), 0.0, 1.0) * coords.w;
