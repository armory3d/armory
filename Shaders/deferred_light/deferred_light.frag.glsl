...
void main() {
	...
	float voxelSize = pow(2.0, clipmapLevel) * 2.0 * voxelgiStep / voxelgiResolution.x;
	vec3 eyeSnap = floor((normalize(viewerPos) * clipmapLevelSize + normalize(eyeLook) * clipmapLevelSize) / voxelSize) * voxelSize;
