...
// Envmap
	...
	if (roughness < 1.0) {
		#ifdef _VoxelGITemporal
		envl += (traceReflection(voxels, voxpos, n, normalize(viewPos), roughness) * voxelBlend + traceReflection(voxels, voxpos, n, normalize(viewPos), roughness) * (1.0 - voxelBlend)) * voxelgiRefl;
		#else
		envl += traceReflection(voxels, voxpos, n, normalize(viewPos), roughness) * voxelgiRefl;
		#endif
	}
...
//	#endif
	...
	float opac = gr.y;
	#ifdef _VoxelGITemporal
	vec3 refraction = (traceRefraction(voxels, voxpos, n, normalize(viewPos), 0.1, rior) * voxelBlend + traceRefraction(voxels, voxpos, n, v, 0.1, rior) * (1.0 - voxelBlend)) * voxelgiRefr;
	#else
	vec3 refraction = traceRefraction(voxels, voxpos, n, normalize(viewPos), 0.1, rior) * voxelgiRefr;
	#endif
