...
void main() {
	...
	float voxelSize = pow(2.0, clipmapLevel) * 0.125;
	vec3 eyeSnap = normalize(floor((viewerPos + eyeLook * clipmapLevelSize) / voxelSize) * voxelSize);
	vec3 voxpos = (normalize(p) - eyeSnap) / voxelSize;
#endif
