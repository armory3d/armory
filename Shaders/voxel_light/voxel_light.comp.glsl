...
void main() {
	...
	for (int i = 0; i < 6; i++) {
		...
		vec3 wnormal = decNor(imageLoad(voxels, src + ivec3(0, 0, voxelgiResolution.x * 3)).r);
	...
	if (lightShadow == 1) {
		...
		if (i == 5) {
			...
			if (direction_weights.x > 0) {
				imageAtomicMax(voxelsLight, src + ivec3(face_offsets.x, 0, 0), convVec4ToRGBA8(vec4(light * direction_weights.x, 1.0)));
			}

			if (direction_weights.y > 0) {
				imageAtomicMax(voxelsLight, src + ivec3(face_offsets.y, 0, 0), convVec4ToRGBA8(vec4(light * direction_weights.y, 1.0)));
			}

			if (direction_weights.z > 0) {
				imageAtomicMax(voxelsLight, src + ivec3(face_offsets.z, 0, 0), convVec4ToRGBA8(vec4(light * direction_weights.z, 1.0)));
			}
