...
vec3 sampleLight(const vec3 p, const vec3 n, const vec3 v, const float dotNV, const vec3 lp, const vec3 lightCol,
	...
	vec3 direct = (albedo + diffuse) * ltcdiff + (ltcspec + reflection) * spec * 0.05;
	#else
	vec3 direct = lambertDiffuseBRDF(albedo, dotNL) + diffuse + (specularBRDF(f0, rough, dotNL, dotNH, dotNV, dotVH) + reflection) * spec;
	#endif
