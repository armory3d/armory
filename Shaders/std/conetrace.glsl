...
vec4 traceCone(sampler3D voxels, vec3 origin, vec3 dir, const float aperture, const float maxDist, const int clipmapLevel) {
    ...
    while (sampleCol.a < 1.0 && dist < maxDist) {
		...
		if(clipmap_blend > 0) {
			...
			vec4 mipSampleNext = textureLod(voxels, (origin + dir * (dist + diam * voxelgiStep)) * 0.5 + 0.5, max(log2(max(voxelSize0, (dist + diam * voxelgiStep) * 2.0 * tan(aperture * 0.5)) * voxelgiResolution.x), 0.0));
...
float traceConeAO(sampler3D voxels, vec3 origin, vec3 dir, const float aperture, const float maxDist, const int clipmapLevel) {
    ...
    while (sampleCol < 1.0 && dist < maxDist) {
		...
		if(clipmap_blend > 0) {
			//Compute sample with next diam.
			float mipSampleNext = textureLod(voxels, (origin + dir * (dist + diam * voxelgiStep)) * 0.5 + 0.5,  max(log2(max(voxelSize0, (dist + diam * voxelgiStep) * 2.0 * tan(aperture * 0.5)) * voxelgiResolution.x), 0.0)).r;
			mipSample = mix(mipSample, mipSampleNext, clipmap_blend);
		}
