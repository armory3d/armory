...
uniform layout(binding = 3, rgba16) writeonly image3D voxelsVr;
...
void main() {
    ...
    vec4 viewRough = imageLoad(voxelsNor, ivec3(gl_GlobalInvocationID.xyz));

    col.rgb += (traceDiffuse(voxpos, wnormal, voxelsFrom).rgb * voxelgiDiff + traceReflection(voxelsFrom, voxpos, wnormal, -viewRough.xyz, viewRough.a).rgb * voxelgiRefl) * 0.5;
